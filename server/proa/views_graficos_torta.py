...
def grafico_torta_materia(request, materia_id):
    ...
    if total_calificaciones > 0:
        porcentajes['1_3'] = calificaciones.filter(nota__range=(1, 3)).count() / total_calificaciones * 100
        porcentajes['4_6'] = calificaciones.filter(nota__range=(4, 6)).count() / total_calificaciones * 100
        porcentajes['7_8'] = calificaciones.filter(nota__range=(7, 8)).count() / total_calificaciones * 100
        porcentajes['9_10'] = calificaciones.filter(nota__range=(9, 10)).count() / total_calificaciones * 100
