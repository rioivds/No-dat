...
from proa.models import Alumno, Cursos
...
def guardar_alumnos(request):
    ...
    insert = Alumno(nombre = nombre, apellido = apellido, email = email, documento = DNI, curso = Cursos.objects.get(id=curso))
    insert.save()
    alumnos = Alumno.objects.all()
...
def guardar_edit(request):
    ...
    curso = request.POST ["curso"]
    alumnos = Alumno.objects.all()
    Alumno.objects.filter(id = id).update(documento = DNI, nombre = nombre, apellido = apellido, email = email, curso = Cursos.objects.get(id=curso))
